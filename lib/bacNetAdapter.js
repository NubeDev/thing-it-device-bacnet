...
module.exports = {
    ...
    createDevice: function (id, ip, port, objectId, vendorId, matchCriteria) {
        return new Device(id, ip, port, objectId, vendorId, matchCriteria);
    }
...
BACnetAdapter.prototype.initialize = function (device) {
    ...
    if ((!device) || (!device.id) || (!device.port)) {
        ...
        if (!this.adapterPort) {
            ...
            this.udpClient.on('listening', function () {
                ...
                //Todo gather promises to resolve at this point rather than ugly wait loop below.
            ...
            //Todo remove and replace with promises that resolve on socket init
            ...
            //nothing needs to be done here anymore
            this.consumers[device.port]++;
        }
        else {
...
BACnetAdapter.prototype.confirmViaWhoIs = function (device) {
    ...
    this.whoIs(device)
        .then(function (result) {
            ...
            device.ip = result.remoteIp;
            device.port = result.remotePort;
            device.vendorId = result.vendorId;
...
BACnetAdapter.prototype.handleMessage = function (message, remote) {
    ...
    if (messageJSON.serviceChoice == BACNET_SERVICES.readProperty
        ...
        if (matchingRequest) {
            ...
            console.log('DEBUG bacNetAdapter: could not find request for invokeId "' + messageJSON.invokeId +
                '" with service choice ' + messageJSON.serviceChoice + '.');
        }
        ...
        var whoIsRequest = _.find(this.requests, {serviceChoice: BACNET_SERVICES.whoIs});

        if (whoIsRequest) {
            console.log('DEBUG bacNetAdapter: (' + remote.address + ':' + remote.port + ') Found request');
            var remoteDevice = new Device(undefined, remote.address, remote.port, messageJSON.objectId, messageJSON.vendorId);

            if (_.isMatch(remoteDevice, whoIsRequest.device.matchCriteria)) {
                messageJSON.remoteIp = remote.address;
                messageJSON.remotePort = remote.port;
                whoIsRequest.resolve(messageJSON);
            } else {
                console.log('DEBUG bacNetAdapter: (' + remote.address + ':' + remote.port + ') Responding device ' +
                    'is not a match to configured device.', remoteDevice, whoIsRequest.device.matchCriteria);
            }
        }
...
function Device(id, ip, port, objectId, vendorId, matchCriteria) {
    this.id = id;
    this.ip = ip;
    this.port = port;
