...
BACnetAdapter.prototype.initialize = function () {
    ...
    this.bacnetStack.on('read-property-ack', function (property) {
        console.log('incoming read-property-ack');
    ...
    this.bacnetStack.on('write-property-ack', function (property) {
        console.log('incoming write-property-ack');
        console.log(property);
    }.bind(this));

    this.bacnetStack.on('ack', function (invokeId, response) {
...
BACnetAdapter.prototype.sendRequest = function (messageJSON, callback) {
    ...
    */
    ...
    if (messageJSON.type == 'RP') {
        ...
        this.bacnetStack.readProperty(this.device.configuration.ipAddress, messageJSON.objectType, messageJSON.objectId, messageJSON.propertyId, false, function (err, response) {
            if (err) {
            ...
            messageJSON.propertyValue = response.value;

            deferred.resolve(messageJSON);
        }.bind(this));
    }

    if (messageJSON.type == 'WP') {
        this.bacnetStack.writeProperty(this.device.configuration.ipAddress, messageJSON.objectType, messageJSON.objectId, messageJSON.propertyId, false, messageJSON.propertyValue, function (err, response) {
            if (err) {
                //this.requests = _.without(this.requests, request);
                deferred.reject();
                throw err;
    ...
    /*
    this.udpClient.send(request.messageBody, 0, request.messageBody.length, BACNET_DEFAULT_PORT, this.device.configuration.ipAddress, function (err, bytes) {
        if (err) {
            this.requests = _.without(this.requests, request);
            throw err;
        }
 ...
 * @returns {*}
 */
BACnetAdapter.prototype.writeProperty = function (objectType, objectId, propertyId, propertyValue) {
...
BACnetAdapter.prototype.subscribeCOV = function (objectType, objectId, propertyId, callback) {
...
BACnetAdapter.prototype.unsubscribeCOV = function (objectType, objectId, propertyId) {
    console.log('adapter unsubscribe to change of value');
    var matchingListener = _.find(this.listeners, {objectType: objectType, objectId: objectId, propertyId: propertyId});
