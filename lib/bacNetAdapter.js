...
BACnetAdapter.prototype.initialize = function (deviceIpAddress, devicePort) {
    ...
    console.log('DEBUG bacNetAdapter: Initializing against IP ' + deviceIpAddress + ' and port ' + devicePort + ".");
        ...
        this.udpClient.on('listening', function () {
            ...
            this.whoIs(device)
                .then(function (result) {
                    ...
                    console.log('DEBUG bacNetAdapter: Initialize complete. Device count: ' + this.devicesMap.size);
                    deferred.resolve();
                }.bind(this))
...
BACnetAdapter.prototype.sendRequest = function (request, callback) {
    ...
    this.udpClient.send(request.message, 0, request.message.length, request.device.port, address, function (err, bytes) {
        ...
        console.log('DEBUG bacNetAdapter: UDP client message sent to ' + address + ':' + request.device.port);
...
Request.prototype.reject = function (message) {
    console.log('DEBUG bacNetAdapter: request reject "' + message + '"');
    clearTimeout(this.timeout);
    this.adapter.requests = _.without(this.adapter.requests, this);
