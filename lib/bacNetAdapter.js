 ...
 * @param port
 */
BACnetAdapter.prototype.initialize = function (deviceIpAddress, devicePort) {
    var deferred = q.defer();
    var device = {
        ip: deviceIpAddress,
        port: devicePort,
        bmsConfig: null,
        id: deviceIpAddress + ':' + devicePort
    };

    console.log('DEBUG bacNetAdapter: (' + device.id + ') Initializing.');

    function confirmViaWhoIs() {
        this.whoIs(device)
            .then(function (result) {
                ...
                console.log('DEBUG bacNetAdapter: (' + device.id + ') Initialize complete. Device count: ' + this.devicesMap.size);
                deferred.resolve();
            }.bind(this))
    ...
    //TODO allow for more than one port and crete multiple UDP listeners.
    if (!this.adapterPort) {
        this.adapterPort = devicePort;
    ...
    } else if (this.adapterPort == devicePort) {
        this.devicesMap.set(device.id, device);
        confirmViaWhoIs.call(this);
    }
    else {
...
BACnetAdapter.prototype.release = function (deviceIpAddress, devicePort) {
    devicesMap.delete(deviceIpAddress + ':' + devicePort);
...
BACnetAdapter.prototype.handleMessage = function (message, remote) {
    ...
    if (messageJSON.serviceChoice == BACNET_SERVICES.readProperty
        ...
        //console.log('DEBUG bacNetAdapter: (' + remote.address + ':' + remote.port + ') Reading i am.');
        var device = this.devicesMap.get(remote.address + ':' + remote.port);

        if (device) {
            console.log('DEBUG bacNetAdapter: (' + device.id + ') Got i am.');
            var matchingRequest = _.find(this.requests, {serviceChoice: BACNET_SERVICES.whoIs});

            if (matchingRequest) {
                console.log('DEBUG bacNetAdapter: (' + device.id + ') Found request');
                matchingRequest.resolve(messageJSON);
            }
        } else {
            //console.log('DEBUG bacNetAdapter: (' + remote.address + ':' + remote.port + ') i am from unknown device.');
        }
...
BACnetAdapter.prototype.readProperty = function (objectTypeString, objectId, propertyIdString, ip, port) {
    console.log('DEBUG bacNetAdapter: (' + ip + ':' + port + ') adapter - read property');
    var promise;

    var device = this.devicesMap.get(ip + ':' + port);
    ...
    if (device) {
        ...
        promise = q.fcall(function () {
            throw new Error("Could not read for unknown device " + ip + ':' + port
                + ". Ensure initialize was called and successful");
        })
    }
...
BACnetAdapter.prototype.writeProperty = function (objectTypeString, objectId, propertyIdString, propertyValue, ip, port) {
    console.log('DEBUG bacNetAdapter: (' + ip + ':' + port + ') adapter - write property');
    ...
    if (device) {
        ...
        promise = q.fcall(function () {
            throw new Error("Could not write for unknown device " + ip + ':' + port
                + ". Ensure initialize was called and successful");
        })
    }
...
BACnetAdapter.prototype.subscribeCOV = function (objectTypeString, objectId, ip, port, callback) {
    console.log('DEBUG bacNetAdapter: (' + ip + ':' + port + ') adapter subscribe to change of value');
    var promise;
    ...
    if (device) {
        ...
        promise = q.fcall(function () {
            throw new Error("Could not subscribe for unknown device " + ip + ':' + port
                + ". Ensure initialize was called and successful");
        })
    }
...
BACnetAdapter.prototype.unsubscribeCOV = function (objectTypeString, objectId, ip, port) {
    console.log('DEBUG bacNetAdapter: (' + ip + ':' + port + ') adapter unsubscribe to change of value');
    var promise;
    ...
    if (device) {
        ...
        promise = q.fcall(function () {
            throw new Error("Could not un-subscribe for unknown device " + ip + ':' + port
                + ". Ensure initialize was called and successful");
        })
    }
